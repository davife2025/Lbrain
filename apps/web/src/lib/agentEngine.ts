...
class AgentEngine {
  ...
  configure(apiKey: string, apiSecret: string, autoTrade: boolean) {
    this.apiKey    = apiKey
  ...
  private async evaluate(rule: AgentRule) {
  ...
  private async executeAction(rule: AgentRule, price: number): Promise<{ success: boolean; message: string }> {
    switch (rule.action) {
      case 'send_alert': {
  ...
  private updateRuleLastRun(id: string) {
    try {
      const saved = localStorage.getItem('lbrain-rules')
      if (!saved) return
      const rules: AgentRule[] = JSON.parse(saved)
