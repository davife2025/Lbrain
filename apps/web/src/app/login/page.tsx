...
export default function LoginPage() {
  ...
  async function loginAsGuest() {
    setGuestLoad(true); reset()
    const res = await signIn('credentials', { anonymous: 'true', redirect: false })
    if (res?.ok) router.push('/')
    else { setError('Guest login failed.'); setGuestLoad(false) }
  }
  ...
  const focus = (e: React.FocusEvent<HTMLInputElement>) => (e.target.style.borderColor = 'var(--blue)')
        ...
        <div className="rounded-2xl p-6 flex flex-col gap-4" style={{ background: 'var(--bg2)', border: '1px solid var(--border)' }}>
          ...
          {mode !== 'reset' && (
            <>
              <button onClick={loginWithGoogle} disabled={googleLoad}
                className="flex items-center justify-center gap-3 py-3 rounded-xl text-sm font-semibold w-full"
                style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', color: 'var(--text)', opacity: googleLoad ? 0.6 : 1, cursor: googleLoad ? 'not-allowed' : 'pointer' }}>
                {googleLoad
              ...
              <button onClick={loginAsGuest} disabled={guestLoad}
                className="flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold w-full"
                style={{ background: 'var(--bg3)', border: '1px solid var(--border)', color: 'var(--text2)', opacity: guestLoad ? 0.6 : 1, cursor: guestLoad ? 'not-allowed' : 'pointer' }}>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {mode === 'login' && (
                  <button type="button" onClick={() => { setMode('reset'); reset() }}
                    className="mono text-[10px] self-end" style={{ color: 'var(--text3)' }}>Forgot password?</button>
                )}
              </div>
            )}
            ...
            <button type="submit" disabled={loading}
              ...
              style={{ background: 'var(--blue)', color: '#fff', opacity: loading ? 0.6 : 1, cursor: loading ? 'not-allowed' : 'pointer' }}>
