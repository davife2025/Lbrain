...
export default function CandleChart({ candles, height = 240, symbol }: Props) {
  ...
  function draw(canvas: HTMLCanvasElement, W: number, H: number, dpr: number) {
    ...
    const cx    = (i: number) => PAD.left + (i + 0.5) * (chartW / candles.length)
    ...
    ctx.fillStyle = lc
    ctx.beginPath()
    ;(ctx as any).roundRect?.(PAD.left + chartW + 4, lastY - tagH/2, tagW, tagH, 3)
    ctx.fill()
    ctx.fillStyle    = '#fff'
  ...
  return (
    <div ref={wrapRef} style={{ width:'100%', position:'relative', userSelect:'none' }}>
      {/* OHLCV tooltip */}
      {showCross && hover && (
        <div style={{ position:'absolute', top:4, left:PAD.left+4, zIndex:10, display:'flex', gap:8, alignItems:'center', fontFamily:'"DM Mono",monospace', fontSize:9, background:'rgba(9,9,15,0.9)', padding:'4px 8px', borderRadius:5, border:'1px solid rgba(255,255,255,0.08)', pointerEvents:'none', whiteSpace:'nowrap' }}>
      ...
      <canvas
        ...
        onMouseLeave={() => { setShowCross(false); setHover(null) }}
      />
    </div>
  )
}
