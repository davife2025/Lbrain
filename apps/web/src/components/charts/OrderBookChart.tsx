...
import type { LiveOrderBook } from '@/hooks/useLBankOrderBook'
...
export default function OrderBookChart({ book, height = 120 }: Props) {
  ...
  return (
    <div style={{ width:'100%', fontFamily:"'DM Mono','Space Mono',monospace" }}>
      {/* Mid price + spread */}
      ...
      <div style={{ position:'relative', height, overflow:'hidden', borderRadius:6, background:'var(--bg3)' }}>
        ...
        <div style={{ position:'absolute', left:0, top:0, bottom:0, width:'50%', display:'flex', alignItems:'flex-end', gap:1, padding:'0 2px', flexDirection:'row-reverse' }}>
          {book.bids.slice(0,15).map((b, i) => (
            <div key={i} style={{ flex:1, height:`${(b.total/maxTotal)*100}%`, background:'rgba(26,111,255,0.5)', borderRadius:'2px 2px 0 0', minWidth:4 }} />
          ))}
        </div>
        ...
        <div style={{ position:'absolute', right:0, top:0, bottom:0, width:'50%', display:'flex', alignItems:'flex-end', gap:1, padding:'0 2px' }}>
          {book.asks.slice(0,15).map((a, i) => (
            <div key={i} style={{ flex:1, height:`${(a.total/maxTotal)*100}%`, background:'rgba(240,79,90,0.5)', borderRadius:'2px 2px 0 0', minWidth:4 }} />
          ))}
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, marginTop:10 }}>
        ...
        <div>
          ...
          {book.bids.slice(0,5).map((b, i) => (
            <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 1fr', fontSize:9, padding:'2px', borderRadius:3, marginBottom:1, background:`rgba(26,111,255,${0.04 + (0.04 * (5-i))})` }}>
        ...
        <div>
          ...
          {book.asks.slice(0,5).map((a, i) => (
            <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 1fr', fontSize:9, padding:'2px', borderRadius:3, marginBottom:1, background:`rgba(240,79,90,${0.04 + (0.04 * (5-i))})` }}>
