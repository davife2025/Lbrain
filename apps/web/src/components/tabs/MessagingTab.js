...
export default function MessagingTab() {
    ...
    const inp = { background: 'var(--bg3)', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: 8, padding: '8px 12px', fontSize: 11, outline: 'none', width: '100%', fontFamily: 'inherit' };
    return (<div style={{ padding: '20px 16px 40px', fontFamily: "'DM Mono','Space Mono',monospace", maxWidth: 520, margin: '0 auto' }}>
      ...
      <div style={{ background: 'rgba(26,111,255,0.06)', border: '1px solid rgba(26,111,255,0.15)', borderRadius: 12, padding: '14px 16px', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {COMMANDS.map(c => (<div key={c.cmd} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {SETUP_STEPS.map(step => (<div key={step.n} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
      ...
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          <input value={appUrl} onChange={e => setAppUrl(e.target.value)} placeholder="https://your-app.vercel.app" style={inp} onFocus={e => e.target.style.borderColor = 'var(--blue)'} onBlur={e => e.target.style.borderColor = 'var(--border)'}/>
        </div>

        <div style={{ marginBottom: 8 }}>
          <div style={{ fontSize: 9, color: 'var(--text3)', marginBottom: 4 }}>OPENCLAW SECRET</div>
          <input value={secret} onChange={e => setSecret(e.target.value)} placeholder="Your OPENCLAW_SECRET value" style={inp} onFocus={e => e.target.style.borderColor = 'var(--blue)'} onBlur={e => e.target.style.borderColor = 'var(--border)'}/>
        </div>

        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 9, color: 'var(--text3)', marginBottom: 4 }}>TELEGRAM BOT TOKEN</div>
          <input value={botToken} onChange={e => setBotToken(e.target.value)} placeholder="123456:ABCdef..." style={inp} onFocus={e => e.target.style.borderColor = 'var(--blue)'} onBlur={e => e.target.style.borderColor = 'var(--border)'}/>
          <div style={{ fontSize: 9, color: 'var(--text3)', marginTop: 4 }}>
            Get from <span style={{ color: 'var(--blue)' }}>@BotFather</span> on Telegram
          </div>
        </div>

        {testRes && (<div style={{ padding: '8px 12px', borderRadius: 8, marginBottom: 10, background: testRes.success ? 'rgba(0,192,135,0.08)' : 'rgba(240,79,90,0.08)', border: `1px solid ${testRes.success ? 'rgba(0,192,135,0.25)' : 'rgba(240,79,90,0.25)'}`, color: testRes.success ? 'var(--green)' : 'var(--red)', fontSize: 10 }}>
            {testRes.message}
          </div>)}

        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={testGateway} disabled={testing || !appUrl || !secret} style={{ flex: 1, padding: '8px', borderRadius: 8, background: 'var(--bg3)', border: '1px solid var(--border)', color: 'var(--text2)', fontSize: 11, cursor: testing || !appUrl || !secret ? 'not-allowed' : 'pointer', opacity: !appUrl || !secret ? 0.5 : 1 }}>
      ...
      {/* Webhook command */}
      {webhookCmd && (<div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text)' }}>Set Telegram Webhook</div>
            <button onClick={() => copy(webhookCmd, 'webhook')} style={{ fontSize: 9, padding: '3px 8px', borderRadius: 5, background: copied === 'webhook' ? 'rgba(0,192,135,0.1)' : 'var(--bg3)', color: copied === 'webhook' ? 'var(--green)' : 'var(--text2)', border: '1px solid var(--border)', cursor: 'pointer' }}>
      ...
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 14, textAlign: 'center' }}>
        ...
        <a href="https://openclaw.ai" target="_blank" rel="noreferrer" style={{ fontSize: 11, color: 'var(--blue)', textDecoration: 'none', fontWeight: 600 }}>
          openclaw.ai →
        </a>
      </div>
    </div>);
}
//# sourceMappingURL=MessagingTab.js.map