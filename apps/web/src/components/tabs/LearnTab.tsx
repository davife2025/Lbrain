...
export default function LearnTab() {
  ...
  async function askQuestion(q: string) {
    ...
    setLoading(true); setAnswer(''); setAsked(query)
    ...
    <div style={{ padding:'20px 16px 40px', fontFamily:"'DM Mono','Space Mono',monospace", maxWidth:600, margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:20 }}>
        <input
          ...
          placeholder="Ask anything about crypto, LBank, trading..."
          style={{ flex:1, background:'var(--bg2)', border:'1px solid var(--border)', color:'var(--text)', borderRadius:10, padding:'10px 14px', fontSize:12, outline:'none', fontFamily:'inherit' }}
          onFocus={e => (e.target as HTMLElement).style.borderColor='var(--blue)'}
          onBlur={e  => (e.target as HTMLElement).style.borderColor='var(--border)'}
        />
        <button onClick={() => askQuestion('')} disabled={loading || !question.trim()}
          style={{ padding:'10px 16px', borderRadius:10, background: loading||!question.trim() ? 'var(--bg3)' : 'var(--blue)', border:'none', color: loading||!question.trim() ? 'var(--text3)' : '#fff', cursor: loading||!question.trim() ? 'not-allowed' : 'pointer', fontSize:12, fontWeight:600 }}>
          Ask
        </button>
      </div>
      ...
      {(answer || loading) && (
        <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:12, padding:'16px', marginBottom:20 }}>
          ...
          {loading && !answer ? (
            ...
            <div style={{ fontSize:12, color:'var(--text)', lineHeight:1.7, whiteSpace:'pre-wrap' }}>{answer}</div>
          )}
        </div>
      )}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
        {TOPICS.map(t => (
          <button key={t.title} onClick={() => { setQuestion(t.q); askQuestion(t.q) }}
            style={{ display:'flex', alignItems:'flex-start', gap:8, padding:'12px', borderRadius:10, background:'var(--bg2)', border:'1px solid var(--border)', cursor:'pointer', textAlign:'left' }}
