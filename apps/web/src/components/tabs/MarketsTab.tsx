...
export default function MarketsTab() {
  ...
  async function fetchCandles(sym: string, iv: string) {
    ...
    try {
      ...
      const json = await res.json()
      if (json.success && Array.isArray(json.data)) setCandles(json.data)
    } catch {}
    setCandleLoad(false)
  }
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', fontFamily:'var(--font-sans)' }}>
      ...
      <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--border)', background:'var(--bg2)', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
          ...
          <button onClick={fetchAll}
            style={{ padding:'5px 10px', borderRadius:7, background:'var(--bg3)', border:'1px solid var(--border)', color:'var(--text2)', fontSize:9, cursor:'pointer', fontFamily:'var(--font-mono)' }}>
            ↻ Refresh
          </button>
        </div>
      ...
      {selected && (
        <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--border)', background:'var(--bg2)', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
            ...
            <button onClick={() => { setSelected(null); setCandles([]) }}
              style={{ fontSize:12, color:'var(--text3)', background:'transparent', border:'none', cursor:'pointer', padding:4 }}>✕</button>
          </div>
          ...
          {/* Candle chart */}
          {candleLoad ? (
            <div style={{ height:220, display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg3)', borderRadius:10, marginBottom:12 }}>
              <div style={{ display:'flex', gap:4 }}>
                <span className="typing-dot" /><span className="typing-dot" /><span className="typing-dot" />
              </div>
            </div>
          ) : candles.length > 0 ? (
            <div style={{ background:'var(--bg3)', borderRadius:10, border:'1px solid var(--border)', padding:'10px', marginBottom:12, overflow:'hidden' }}>
              <CandleChart candles={candles} symbol={selected} height={220} />
            </div>
          ) : (
            <div style={{ height:80, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text3)', fontSize:11, marginBottom:12 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 70px', padding:'6px 16px', borderBottom:'1px solid var(--border)', flexShrink:0 }}>
        {['Pair','Price','24h %','Volume'].map(h => (
          <div key={h} style={{ fontSize:8, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'0.06em', textAlign:h==='Pair'?'left':'right', fontFamily:'var(--font-mono)' }}>
            {h}
          </div>
        ))}
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto' }}>
        {loading ? (
          ...
          <div style={{ padding:24, textAlign:'center' }}>
            ...
            <button onClick={fetchAll}
              style={{ padding:'6px 14px', borderRadius:8, background:'var(--blue)', color:'#fff', border:'none', cursor:'pointer', fontSize:11 }}>
              Retry
            </button>
          </div>
        ) : filtered.length === 0 ? (
            ...
            return (
              <div key={t.symbol}
                ...
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  ...
                  {isLive && (
                    <div style={{ width:4, height:4, borderRadius:'50%', background:'var(--green)', animation:'lbP 2s infinite', flexShrink:0 }} />
                  )}
                </div>
