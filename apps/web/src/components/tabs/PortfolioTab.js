...
export default function PortfolioTab() {
    ...
    const getLive = (coin) => tickers[`${coin.toLowerCase()}_usdt`]?.price ?? null;
    ...
    return (<div style={{ padding: '20px 16px 40px', fontFamily: "'DM Mono','Space Mono',monospace", maxWidth: 560, margin: '0 auto' }}>
      ...
      {holdings.length > 0 && (<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 20 }}>
          {[['Value', fmt(totalValue), 'var(--text)'], ['Cost', fmt(totalCost), 'var(--text2)'], ['PnL', `${totalPnL >= 0 ? '+' : ''}${fmt(totalPnL)}`, totalPnL >= 0 ? 'var(--green)' : 'var(--red)']].map(([l, v, c]) => (<div key={l} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 12px' }}>
              <div style={{ fontSize: 9, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 3 }}>{l}</div>
              <div style={{ fontSize: 12, fontWeight: 700, color: c }}>{v}</div>
            </div>))}
        </div>)}

      {donutData.length > 1 && (<div style={{ display: 'flex', alignItems: 'center', gap: 16, background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px', marginBottom: 20 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5 }}>
            {donutPaths.map(d => (<div key={d.coin} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {adding && (<div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 10 }}>
            <div><div style={{ fontSize: 9, color: 'var(--text3)', marginBottom: 4 }}>COIN</div><input value={coin} onChange={e => setCoin(e.target.value)} placeholder="BTC" style={inp} onFocus={e => e.target.style.borderColor = 'var(--blue)'} onBlur={e => e.target.style.borderColor = 'var(--border)'}/></div>
            <div><div style={{ fontSize: 9, color: 'var(--text3)', marginBottom: 4 }}>QTY</div><input type="number" value={qty} onChange={e => setQty(e.target.value)} placeholder="0.5" style={inp} onFocus={e => e.target.style.borderColor = 'var(--blue)'} onBlur={e => e.target.style.borderColor = 'var(--border)'}/></div>
            <div><div style={{ fontSize: 9, color: 'var(--text3)', marginBottom: 4 }}>AVG BUY $</div><input type="number" value={avgBuy} onChange={e => setAvgBuy(e.target.value)} placeholder="50000" style={inp} onFocus={e => e.target.style.borderColor = 'var(--blue)'} onBlur={e => e.target.style.borderColor = 'var(--border)'}/></div>
          </div>
      ...
      {holdings.length === 0 ? (<div style={{ textAlign: 'center', padding: '48px 0' }}>
          ...
          {holdings.map(h => {
                ...
                return (<div key={h.coin} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px' }}>
