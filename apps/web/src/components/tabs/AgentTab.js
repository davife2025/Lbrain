...
export default function AgentTab() {
    ...
    function toggleRule(id) { setRules(r => r.map(rule => rule.id === id ? { ...rule, active: !rule.active } : rule)); }
    function deleteRule(id) { setRules(r => r.filter(rule => rule.id !== id)); }
    const inp = { background: 'var(--bg3)', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: 8, padding: '8px 12px', fontSize: 12, outline: 'none', width: '100%', fontFamily: 'inherit' };
    ...
    return (<div style={{ padding: '20px 16px 40px', fontFamily: "'DM Mono','Space Mono',monospace", maxWidth: 520, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={() => setShowLog(v => !v)} style={{ padding: '6px 12px', borderRadius: 8, background: showLog ? 'rgba(26,111,255,0.15)' : 'var(--bg2)', border: `1px solid ${showLog ? 'rgba(26,111,255,0.3)' : 'var(--border)'}`, color: showLog ? 'var(--blue)' : 'var(--text2)', fontSize: 10, cursor: 'pointer' }}>
      ...
      {/* Auto-trade warning */}
      {!autoTradeEnabled && (<div style={{ background: 'rgba(240,185,11,0.06)', border: '1px solid rgba(240,185,11,0.2)', borderRadius: 10, padding: '10px 14px', marginBottom: 14, fontSize: 10, color: '#f59e0b' }}>
          ⚠ Auto-trade is OFF — trading actions will be skipped. Enable in Settings.
        </div>)}

      {/* Execution logs */}
      {showLog && (<div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          ...
          {logs.length === 0 ? (<div style={{ fontSize: 10, color: 'var(--text3)', textAlign: 'center', padding: '12px 0' }}>No executions yet</div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 200, overflowY: 'auto' }}>
              {logs.map((log, i) => (<div key={i} style={{ padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.03)', border: `1px solid ${log.success ? 'rgba(0,192,135,0.15)' : 'rgba(240,79,90,0.15)'}` }}>
      ...
      {adding && (<div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. BTC Take Profit" style={inp} onFocus={e => e.target.style.borderColor = 'var(--blue)'} onBlur={e => e.target.style.borderColor = 'var(--border)'}/>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
            ...
            <div>
              ...
              <input value={trigVal} onChange={e => setTrigVal(e.target.value)} placeholder="105000" type="number" style={inp} onFocus={e => e.target.style.borderColor = 'var(--blue)'} onBlur={e => e.target.style.borderColor = 'var(--border)'}/>
            </div>
          </div>
      ...
      {/* Rules list */}
      {rules.length === 0 ? (<div style={{ textAlign: 'center', padding: '48px 0' }}>
          ...
          {rules.map(rule => (<div key={rule.id} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px', opacity: rule.active ? 1 : 0.5 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)' }}>{rule.name}</div>
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  {rule.lastRun && (<span style={{ fontSize: 8, color: 'var(--green)', padding: '1px 5px', borderRadius: 3, background: 'rgba(0,192,135,0.1)' }}>ran</span>)}
                  <button onClick={() => toggleRule(rule.id)} style={{ fontSize: 8, padding: '2px 8px', borderRadius: 4, cursor: 'pointer', background: rule.active ? 'rgba(26,111,255,0.1)' : 'rgba(255,255,255,0.05)', color: rule.active ? 'var(--blue)' : 'var(--text3)', border: rule.active ? '1px solid rgba(26,111,255,0.2)' : '1px solid var(--border)' }}>
                    {rule.active ? 'ON' : 'OFF'}
                  </button>
              ...
              {rule.lastRun && (<div style={{ fontSize: 8, color: 'var(--text3)', marginTop: 4 }}>
