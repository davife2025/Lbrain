...
function getSignal(rsi, macd, price, bb) {
    let score = 0;
    const reasons = [];
    if (rsi < 30) {
        score += 2;
        ...
        score += 1;
        reasons.push('RSI leaning bullish');
    }
    else if (rsi > 70) {
        score -= 2;
        ...
        score -= 1;
        reasons.push('RSI leaning bearish');
    }
    if (macd.hist > 0) {
        score += 1;
    ...
    if (score >= 3)
        return { signal: 'STRONG BUY', color: '#00c087', score, reasons };
    else if (score >= 1)
        return { signal: 'BUY', color: '#00c087', score, reasons };
    else if (score <= -3)
        return { signal: 'STRONG SELL', color: '#f04f5a', score, reasons };
    else if (score <= -1)
        return { signal: 'SELL', color: '#f04f5a', score, reasons };
    else
        return { signal: 'NEUTRAL', color: '#f59e0b', score, reasons };
}
export default function TradingTab() {
    ...
    useEffect(() => {
        if (ticker?.price && orderType === 'limit' && !price)
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', height: '100%', fontFamily: 'var(--font-sans)', overflowY: 'auto' }}>
      ...
      <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', background: 'var(--bg2)', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 5, overflowX: 'auto', marginBottom: 10 }}>
          {QUICK_PAIRS.map(p => (<button key={p} onClick={() => { setSymbol(p); setPrice(''); }} style={{ padding: '4px 10px', borderRadius: 6, fontSize: 9, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0, background: symbol === p ? 'var(--blue)' : 'var(--bg3)', color: symbol === p ? '#fff' : 'var(--text2)', border: symbol === p ? 'none' : '1px solid var(--border)', fontFamily: 'var(--font-mono)' }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {TAB_VIEWS.map(t => (<button key={t.id} onClick={() => { setView(t.id); if (t.id === 'orders')
            fetchOpenOrders(); }} style={{ padding: '5px 12px', borderRadius: 7, fontSize: 10, cursor: 'pointer', fontFamily: 'var(--font-sans)', background: view === t.id ? 'var(--blue)' : 'transparent', color: view === t.id ? '#fff' : 'var(--text2)', border: view === t.id ? 'none' : '1px solid var(--border)' }}>
              {t.label}
            </button>))}
        </div>
      </div>
      ...
      {/* ── CHART VIEW ── */}
      {view === 'chart' && (<div style={{ padding: '12px 16px', flexShrink: 0 }}>
          <div style={{ display: 'flex', gap: 4, marginBottom: 10 }}>
            {INTERVALS.map(iv => (<button key={iv.id} onClick={() => setInterval(iv.id)} style={{ padding: '3px 8px', borderRadius: 5, fontSize: 9, cursor: 'pointer', background: interval === iv.id ? 'var(--blue)' : 'var(--bg3)', color: interval === iv.id ? '#fff' : 'var(--text3)', border: interval === iv.id ? 'none' : '1px solid var(--border)', fontFamily: 'var(--font-mono)' }}>
                {iv.label}
              </button>))}
          </div>

          {candleLoad ? (<div style={{ height: 240, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg2)', borderRadius: 10 }}>
              <div style={{ display: 'flex', gap: 4 }}>
                <span className="typing-dot"/><span className="typing-dot"/><span className="typing-dot"/>
              </div>
            </div>) : candles.length > 0 ? (<div style={{ background: 'var(--bg2)', borderRadius: 10, border: '1px solid var(--border)', padding: '10px', overflow: 'hidden' }}>
              <CandleChart candles={candles} symbol={symbol} height={240}/>
            </div>) : (<div style={{ height: 240, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg2)', borderRadius: 10, color: 'var(--text3)', fontSize: 12 }}>
              No chart data available
            </div>)}

          {book && (<div style={{ marginTop: 12 }}>
      ...
      {/* ── ORDER VIEW ── */}
      {view === 'order' && (<div style={{ padding: '16px', flexShrink: 0 }}>
          {!isConnected && (<div style={{ background: 'rgba(26,111,255,0.06)', border: '1px solid rgba(26,111,255,0.2)', borderRadius: 10, padding: '10px 14px', marginBottom: 14, fontSize: 11, color: 'var(--blue)' }}>
              ⚠ Connect your LBank API key in Settings to trade
            </div>)}

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
            {['buy', 'sell'].map(s => (<button key={s} onClick={() => setSide(s)} style={{ padding: '10px', borderRadius: 8, fontSize: 12, fontWeight: 500, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.05em', background: side === s ? (s === 'buy' ? 'var(--green)' : 'var(--red)') : 'var(--bg2)', color: side === s ? '#fff' : 'var(--text2)', border: side === s ? 'none' : '1px solid var(--border)' }}>
                {s}
              </button>))}
          </div>

          <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
            {['limit', 'market'].map(t => (<button key={t} onClick={() => setOrderType(t)} style={{ padding: '5px 14px', borderRadius: 6, fontSize: 10, cursor: 'pointer', textTransform: 'capitalize', background: orderType === t ? 'rgba(26,111,255,0.15)' : 'transparent', color: orderType === t ? 'var(--blue)' : 'var(--text3)', border: orderType === t ? '1px solid rgba(26,111,255,0.3)' : '1px solid var(--border)' }}>
                {t}
              </button>))}
          </div>

          {orderType === 'limit' && (<div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 9, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Price (USDT)</div>
              <input value={price} onChange={e => setPrice(e.target.value)} placeholder="Enter price" type="number" style={inp} onFocus={e => e.target.style.borderColor = 'var(--blue)'} onBlur={e => e.target.style.borderColor = 'var(--border)'}/>
            </div>)}

          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 9, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Amount</div>
            <input value={amount} onChange={e => setAmount(e.target.value)} placeholder="Enter amount" type="number" style={inp} onFocus={e => e.target.style.borderColor = 'var(--blue)'} onBlur={e => e.target.style.borderColor = 'var(--border)'}/>
            <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
              {[25, 50, 75, 100].map(pct => (<button key={pct} onClick={() => setAmount((pct / 100).toFixed(4))} style={{ flex: 1, padding: '3px', borderRadius: 5, fontSize: 9, cursor: 'pointer', background: 'var(--bg3)', color: 'var(--text3)', border: '1px solid var(--border)', fontFamily: 'var(--font-mono)' }}>
                  {pct}%
                </button>))}
            </div>
          </div>

          {orderType === 'limit' && amount && price && (<div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 12px', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, marginBottom: 12 }}>
          ...
          {result && (<div style={{ padding: '9px 12px', borderRadius: 8, marginBottom: 12, background: result.success ? 'rgba(0,192,135,0.08)' : 'rgba(240,79,90,0.08)', border: `1px solid ${result.success ? 'rgba(0,192,135,0.25)' : 'rgba(240,79,90,0.25)'}`, color: result.success ? 'var(--green)' : 'var(--red)', fontSize: 11 }}>
              {result.message}
            </div>)}

          <button onClick={placeOrder} disabled={loading || !isConnected || !amount} style={{ width: '100%', padding: '12px', borderRadius: 10, fontSize: 13, fontWeight: 500, cursor: loading || !isConnected || !amount ? 'not-allowed' : 'pointer', background: !isConnected || !amount ? 'var(--bg3)' : side === 'buy' ? 'var(--green)' : 'var(--red)', color: !isConnected || !amount ? 'var(--text3)' : '#fff', border: 'none', opacity: loading ? 0.7 : 1, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>
      ...
      {/* ── ANALYSE VIEW ── */}
      {view === 'analyse' && (<div style={{ padding: '16px', flexShrink: 0 }}>
          ...
          {candles.length < 20 ? (<div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--text3)', fontSize: 12 }}>
              Need at least 20 candles. Switch to 1H or 4H interval on the Chart tab.
            </div>) : (<>
              {/* Overall signal */}
              {analysis && (<div style={{ background: 'var(--bg2)', border: `1px solid ${analysis.color}33`, borderRadius: 12, padding: '16px', marginBottom: 14, textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center', gap: 3 }}>
                    {[-3, -2, -1, 0, 1, 2, 3].map(v => (<div key={v} style={{ width: 18, height: 6, borderRadius: 3, background: v <= analysis.score ? analysis.color : 'rgba(255,255,255,0.08)' }}/>))}
                  </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
                ...
                ].map(ind => (<div key={ind.label} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 12px' }}>
              ...
              {/* Reasons */}
              {analysis && (<div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ fontSize: 10, fontWeight: 500, color: 'var(--text)', marginBottom: 10 }}>Signal Reasons</div>
                  {analysis.reasons.map((r, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: i < analysis.reasons.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
      ...
      {/* ── OPEN ORDERS VIEW ── */}
      {view === 'orders' && (<div style={{ padding: '16px', flexShrink: 0 }}>
          {!isConnected ? (<div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--text3)', fontSize: 12 }}>Connect your LBank API key in Settings to see orders</div>) : openOrders.length === 0 ? (<div style={{ textAlign: 'center', padding: '32px 0' }}>
              ...
              {openOrders.map(o => (<div key={o.orderId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 14px' }}>
                  <div>
                  ...
                  <button onClick={() => cancelOrder(o.orderId)} style={{ fontSize: 10, padding: '4px 10px', borderRadius: 6, background: 'rgba(240,79,90,0.08)', color: 'var(--red)', border: '1px solid rgba(240,79,90,0.2)', cursor: 'pointer' }}>
                    Cancel
                  </button>
                </div>))}
            </div>)}
...
//# sourceMappingURL=TradingTab.js.map