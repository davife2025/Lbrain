...
function PreviewMarkets() {
    return (<div style={pBase}>
      {[['BTC', '$103,420', '+2.4%', true], ['ETH', '$3,821', '+1.8%', true], ['LTC', '$89.42', '+1.8%', true], ['XRP', '$0.62', '-0.9%', false]].map(([s, v, c, up]) => (<div key={s} style={{ display: 'flex', justifyContent: 'space-between', padding: '2px 0', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
...
function PreviewPortfolio() {
    return (<div style={pBase}>
      {[['BTC', 72, '#1a6fff', '$4,210'], ['ETH', 45, '#00c087', '$1,840'], ['LTC', 22, '#f59e0b', '$420']].map(([s, p, col, v]) => (<div key={s} style={{ marginBottom: 5 }}>
          ...
          <div style={{ width: '100%', height: 2, background: 'rgba(255,255,255,0.05)', borderRadius: 2, marginTop: 2 }}>
            <div style={{ width: `${p}%`, height: 2, borderRadius: 2, background: col }}/>
          </div>
        </div>))}
    </div>);
}
function PreviewAlerts() {
    return (<div style={pBase}>
      {[['BTC', 'above $105k', 'var(--blue)', true], ['ETH', 'below $3.5k', 'var(--red)', true], ['LTC', 'above $100', 'var(--blue)', false]].map(([s, c, col, on]) => (<div key={s} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)', borderRadius: 4, padding: '3px 5px', marginBottom: 3 }}>
          <span style={{ fontSize: 7, color: 'var(--text3)' }}>{s}</span>
          <span style={{ fontSize: 7, color: col }}>{c}</span>
...
function PreviewTrade() {
    return (<div style={pBase}>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 38 }}>
        {[28, 45, 36, 62, 55, 82, 70, 78].map((h, i) => (<div key={i} style={{ flex: 1, height: `${h}%`, borderRadius: '2px 2px 0 0', background: i >= 5 ? 'var(--green)' : i >= 3 ? 'var(--blue)' : 'rgba(255,255,255,0.06)' }}/>))}
      </div>
...
function PreviewAgent() {
    return (<div style={pBase}>
      {[['IF BTC > $105k', 'BUY', 'var(--green)'], ['IF RSI > 80', 'ALERT', 'var(--blue)'], ['DAILY 9AM', 'REPORT', '#8b5cf6']].map(([cond, act, col]) => (<div key={cond} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)', borderRadius: 4, padding: '3px 5px', marginBottom: 3 }}>
          <span style={{ fontSize: 7, color: 'var(--text3)' }}>{cond}</span>
          <span style={{ fontSize: 6, padding: '1px 5px', borderRadius: 3, background: 'rgba(0,0,0,0.3)', color: col, border: `1px solid ${col}33`, fontFamily: 'var(--font-mono)' }}>{act}</span>
        </div>))}
    </div>);
}
const FEAT_COLS = [
...
function useScroll(ref, colW) {
    useEffect(() => {
        ...
        let offset = 0, paused = false, dragging = false, startX = 0, startOff = 0, raf;
        const tick = () => {
            if (!paused && !dragging) {
                offset += 0.35;
                if (offset > total)
        ...
        const down = (e) => { dragging = true; paused = true; startX = e.clientX; startOff = offset; e.preventDefault(); };
        const move = (e) => { if (!dragging)
            return; offset = startOff + (startX - e.clientX); if (offset > total * 2)
            offset -= total; if (offset < 0)
            offset += total; track.style.transform = `translateX(${-offset}px)`; };
        const up = () => { if (dragging) {
            dragging = false;
            paused = false;
        } };
        const ts = (e) => { dragging = true; paused = true; startX = e.touches[0].clientX; startOff = offset; };
        const tm = (e) => { if (!dragging)
            return; offset = startOff + (startX - e.touches[0].clientX); if (offset > total * 2)
            offset -= total; if (offset < 0)
            offset += total; track.style.transform = `translateX(${-offset}px)`; };
...
export default function DashboardTab() {
    ...
    const wrap = { overflow: 'hidden', cursor: 'grab', userSelect: 'none' };
    const track = { display: 'flex', gap: 8, width: 'max-content' };
    const col = { display: 'flex', flexDirection: 'column', gap: 8 };
    const navCard = { width: 108, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, padding: '12px 8px', borderRadius: 10, background: 'var(--bg2)', border: '1px solid var(--border)', cursor: 'pointer', textAlign: 'center' };
    const featCard = { width: 108, borderRadius: 10, background: 'var(--bg2)', border: '1px solid var(--border)', cursor: 'pointer', overflow: 'hidden', textAlign: 'left', padding: 0 };
    const Divider = ({ label }) => (<div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '16px 0 12px' }}>
    ...
    return (<div style={{ minHeight: '100%', background: 'var(--bg)', fontFamily: 'var(--font-sans)' }}>
      <LiveTickerStrip />
        ...
        <div style={{ maxWidth: 840, margin: '0 auto' }}>
          ...
          <div style={wrap}>
            <div ref={navRef} style={track}>
              {NAV_COLS.map((pair, ci) => (<div key={ci} style={col}>
                  {pair.map(item => (<button key={item.id} onClick={() => setActiveTab(item.id)} style={navCard} onMouseEnter={e => { const el = e.currentTarget; el.style.borderColor = 'var(--blue)'; el.style.background = 'var(--bg3)'; }} onMouseLeave={e => { const el = e.currentTarget; el.style.borderColor = 'var(--border)'; el.style.background = 'var(--bg2)'; }}>
        ...
        <div style={{ maxWidth: 840, margin: '0 auto' }}>
          ...
          <div style={wrap}>
            <div ref={featRef} style={track}>
              {FEAT_COLS.map((pair, ci) => (<div key={ci} style={col}>
                  {pair.map(({ id, icon, name: fn, sub, Preview }) => (<button key={id} onClick={() => setActiveTab(id)} style={featCard} onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--blue)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}>
                      <Preview />
