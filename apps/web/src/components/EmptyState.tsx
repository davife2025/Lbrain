...
export default function EmptyState({ icon, title, desc, action, onAction }: Props) {
  return (
    <div style={{ textAlign:'center', padding:'48px 20px', fontFamily:'var(--font-sans)' }}>
      ...
      {action && onAction && (
        <button onClick={onAction}
          style={{ padding:'7px 18px', borderRadius:8, background:'var(--blue)', border:'none', color:'#fff', fontSize:11, cursor:'pointer', fontWeight:500 }}>
          {action}
        </button>
      )}
    </div>
