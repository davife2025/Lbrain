...
class NotificationService {
  ...
  async sendAlertTrigger(trigger: AlertTrigger) {
    const payload = {
  ...
  async sendAgentExecution(exec: RuleExecution) {
    const payload = {
      type:    'agent_executed',
      userId:  exec.userId,
      data:    exec,
  ...
  private async flush() {
    if (this.flushing || !this.queue.length || !WEB_APP_URL) return
    this.flushing = true
  ...
  async sendTelegramMessage(chatId: string, message: string) {
    if (!WEB_APP_URL || !OPENCLAW_SECRET) return
    try {
      await axios.post(
        `${WEB_APP_URL}/api/openclaw/message`,
        { message, channel: 'telegram', sender: 'lbrain-server', chatId },
        {
          headers: {
            'Content-Type':      'application/json',
